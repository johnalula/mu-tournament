	...
	<!-- ************ Account Chart Management ********************** -->
		...
		<?php if($sf_request->getParameter('module') == 'tournament' ): ?>
			...
			<?php if($sf_request->getParameter('action') == 'new'): ?>
				<li class="">
					<button title="<?php echo __('Save Tournament Information') ?>" id="createTournament" class="ui-disabled-toolbar-btn" disabled >
						<img class="navbar-nav-img" src="<?php echo image_path('icons/save') ?>">
						<?php echo __('Save') ?>
					</button>
				</li>	
				<li class="">
					<button title="<?php echo __('Cancel Tournament Information') ?>" id="cancelTournament" class="ui-disabled-toolbar-btn" disabled>
